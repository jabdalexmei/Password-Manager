...
pub fn list_bank_cards(
    ...
    with_connection_in_active_vault(state, profile_id, |conn, active_vault_id| {
        ...
        if fallback {
            log::warn!(
                "[SORT] profile_id={} entity=bank_cards action=list_bank_cards include_deleted={} fallback=true sort_field={} sort_dir={}",
                profile_id,
                include_deleted,
                sort_field,
                sort_dir
        ...
        let base_query = if include_deleted {
            format!("SELECT * FROM bank_cards WHERE vault_id = ?1 {clause}")
        } else {
            format!("SELECT * FROM bank_cards WHERE vault_id = ?1 AND deleted_at IS NULL {clause}")
        };
        let mut stmt = conn
            .prepare(&base_query)
            .map_err(|_| ErrorCodeString::new("DB_QUERY_FAILED"))?;
        let cards = stmt
            .query_map(params![active_vault_id], map_bank_card)
            .map_err(|_| ErrorCodeString::new("DB_QUERY_FAILED"))?
            .collect::<rusqlite::Result<Vec<_>>>()
            .map_err(|_| ErrorCodeString::new("DB_QUERY_FAILED"))?;
        Ok(cards)
