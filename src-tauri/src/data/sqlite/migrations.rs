...
-- Normalize arbitrary legacy values to strict 0/1 before enforcing uniqueness.
UPDATE vaults
SET is_default = CASE WHEN is_default = 1 THEN 1 ELSE 0 END;

-- Deterministic choice for the single default vault:
-- 1) Prefer rows already marked as default.
-- 2) Prefer legacy 'default' id if still present.
-- 3) Then oldest created_at, then id.
WITH chosen AS (
  SELECT id
  FROM vaults
  ORDER BY
...
UPDATE vaults
SET is_default = CASE WHEN id = (SELECT id FROM chosen) THEN 1 ELSE 0 END;
...
"#,
    ...
    .map_err(|e| {
        log_sqlite_err("migrate_v2_to_v3.execute_batch", &e);
        ErrorCodeString::new("DB_MIGRATION_FAILED")
    })?;
