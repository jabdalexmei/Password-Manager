...
const IDX_VAULTS_SINGLE_DEFAULT: &str = "idx_vaults_single_default";
...
fn has_index(conn: &Connection, name: &str) -> Result<bool> {
    let sql = "SELECT 1 FROM sqlite_master WHERE type='index' AND name=?1 LIMIT 1";
    let exists: Option<i32> = conn
        .query_row(sql, [name], |row| row.get(0))
        .optional()
        .map_err(|e| {
            log_sqlite_err(&format!("has_index.query_row name={name} sql={sql}"), &e);
            ErrorCodeString::new("DB_QUERY_FAILED")
        })?;
    Ok(exists.is_some())
}
...
-- Deterministic default selection rule used for normalization:
-- 1) Prefer historical id='default' when present.
-- 2) Otherwise choose the lexicographically earliest by (created_at, id).
WITH chosen AS (
  SELECT id
  FROM vaults
  ORDER BY
...
UPDATE vaults
SET is_default = CASE
  WHEN id = (SELECT id FROM chosen) THEN 1
  ELSE 0
END;
"#,
    )
    .map_err(|e| {
        log_sqlite_err("migrate_v2_to_v3.normalize_default", &e);
        ErrorCodeString::new("DB_MIGRATION_FAILED")
    })?;

    if !has_index(conn, IDX_VAULTS_SINGLE_DEFAULT)? {
        conn.execute(
            "CREATE UNIQUE INDEX idx_vaults_single_default ON vaults(is_default) WHERE is_default = 1;",
            [],
        )
        .map_err(|e| {
            log_sqlite_err("migrate_v2_to_v3.create_idx_vaults_single_default", &e);
            ErrorCodeString::new("DB_MIGRATION_FAILED")
        })?;
    }
...
pub fn migrate_to_latest(conn: &Connection) -> Result<()> {
    ...
    if version == 2 {
        log::info!("[DB][migrate] migrate from=2 to=3");
...
"#,
    ...
    #[test]
    fn migrate_v2_to_v3_prefers_legacy_default_id() {
        let conn = Connection::open_in_memory().unwrap();
        create_vaults_table(&conn);

        conn.execute(
            "INSERT INTO vaults (id, name, is_default, created_at, updated_at) VALUES (?1, ?2, 1, ?3, ?4)",
            ("custom", "Custom", "2026-01-02T00:00:00Z", "2026-01-02T00:00:00Z"),
        )
        .unwrap();
        conn.execute(
            "INSERT INTO vaults (id, name, is_default, created_at, updated_at) VALUES (?1, ?2, 1, ?3, ?4)",
            ("default", "Default vault", "2026-01-03T00:00:00Z", "2026-01-03T00:00:00Z"),
        )
        .unwrap();
        ...
        let default_id: String = conn
            .query_row("SELECT id FROM vaults WHERE is_default = 1", [], |row| row.get(0))
            .unwrap();
        let defaults_count: i64 = conn
            .query_row("SELECT COUNT(*) FROM vaults WHERE is_default = 1", [], |row| row.get(0))
            .unwrap();

        assert_eq!(default_id, "default");
        assert_eq!(defaults_count, 1);
        assert!(has_index(&conn, IDX_VAULTS_SINGLE_DEFAULT).unwrap());
    }

    #[test]
    fn migrate_v2_to_v3_assigns_default_when_missing() {
        let conn = Connection::open_in_memory().unwrap();
        create_vaults_table(&conn);

        conn.execute(
            "INSERT INTO vaults (id, name, is_default, created_at, updated_at) VALUES (?1, ?2, 0, ?3, ?4)",
            ("vault-b", "Vault B", "2026-01-02T00:00:00Z", "2026-01-02T00:00:00Z"),
        )
        .unwrap();
        conn.execute(
            "INSERT INTO vaults (id, name, is_default, created_at, updated_at) VALUES (?1, ?2, 0, ?3, ?4)",
            ("vault-a", "Vault A", "2026-01-01T00:00:00Z", "2026-01-01T00:00:00Z"),
        )
        .unwrap();
