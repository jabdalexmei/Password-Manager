...
fn vault_exists_conn(conn: &Connection, id: &str) -> Result<bool> {
    let exists = conn
        .query_row(
            "SELECT 1 FROM vaults WHERE id = ?1 LIMIT 1",
            params![id],
            |row| row.get::<_, i32>(0),
        )
        .optional()
...
fn resolve_runtime_vault_id_conn(conn: &Connection, state: &Arc<AppState>) -> Result<String> {
    let active_vault_id = current_active_vault_id(state);
    if vault_exists_conn(conn, &active_vault_id)? {
...
fn get_default_vault_by_conn(conn: &Connection) -> Result<Vault> {
    ...
    match conn.query_row(default_sql, [], map_vault) {
        ...
        Err(rusqlite::Error::QueryReturnedNoRows) => {
            // Fallback for legacy/corrupted states: pick one deterministic vault.
            let fallback_sql = "SELECT id, name, is_default, created_at, updated_at FROM vaults ORDER BY CASE WHEN id = ?1 THEN 0 ELSE 1 END, created_at ASC, id ASC LIMIT 1";
            conn.query_row(fallback_sql, params![DEFAULT_VAULT_ID], map_vault)
                .map_err(|_| ErrorCodeString::new("VAULT_NOT_FOUND"))
        }
        Err(_) => Err(ErrorCodeString::new("DB_QUERY_FAILED")),
    }
}
...
pub fn set_default_vault(state: &Arc<AppState>, profile_id: &str, vault_id: &str) -> Result<bool> {
    with_connection(state, profile_id, |conn| {
        ...
        let result: Result<()> = (|| {
            let _ = get_vault_by_id_conn(conn, vault_id)?;
            let now = Utc::now().to_rfc3339();

            conn.execute("UPDATE vaults SET is_default = 0 WHERE is_default <> 0", [])
                .map_err(|_| ErrorCodeString::new("DB_QUERY_FAILED"))?;
