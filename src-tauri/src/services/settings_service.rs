...
pub fn update_settings_command(state: &Arc<AppState>, mut settings: UserSettings) -> Result<bool> {
    ...
    let multiply_vaults_enabled = settings.multiply_vaults_enabled;
    ...
    if updated {
        let runtime_active_vault_id = if multiply_vaults_enabled {
            settings.active_vault_id
        } else {
            repo_impl::get_default_vault(state, &profile_id)
                .map(|vault| vault.id)
