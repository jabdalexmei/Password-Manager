...
pub fn list_bank_cards(state: &Arc<AppState>) -> Result<Vec<BankCardItem>> {
    let profile_id = security_service::require_unlocked_active_profile(state)?.profile_id;
    let storage_paths = state.get_storage_paths()?;
    let settings = get_settings(&storage_paths, &profile_id)?;
    repo_impl::list_bank_cards(
        state,
        &profile_id,
        false,
        &settings.default_sort_field,
...
pub fn list_deleted_bank_cards(state: &Arc<AppState>) -> Result<Vec<BankCardItem>> {
    let profile_id = security_service::require_unlocked_active_profile(state)?.profile_id;
    repo_impl::list_deleted_bank_cards(state, &profile_id)
}
