...
const detectNavigatorLanguage = (): Language => {
  const win = getWindow();
  if (!win) {
  ...
  if (Array.isArray(win.navigator.languages)) {
    candidates.push(...win.navigator.languages);
  }
  if (typeof win.navigator.language === 'string') {
    candidates.push(win.navigator.language);
  }
...
const resolveInitialLanguage = (): Language => readStoredLanguage() ?? detectNavigatorLanguage();
