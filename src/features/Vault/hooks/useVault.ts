...
export function useVault(profileId: string, onLocked: () => void) {
  ...
  const refreshVaults = useCallback(async (): Promise<VaultItem[] | null> => {
    try {
      ...
      const mappedVaults = sortVaultItems(fetchedVaults.map(mapVaultFromBackend));
  ...
  const updateSettingsAction = useCallback(
    async (nextSettings: BackendUserSettings) => {
      try {
        ...
        if (requestedActiveVaultId !== activeVaultId || !nextSettings.multiply_vaults_enabled) {
          await setActiveVault(requestedActiveVaultId);
        }

        const refreshedVaults = await refreshVaults();
        const defaultVaultId =
          refreshedVaults?.find((vaultItem) => vaultItem.isDefault)?.id ?? DEFAULT_ACTIVE_VAULT_ID;
        const runtimeActiveVaultId = nextSettings.multiply_vaults_enabled
          ? requestedActiveVaultId
          : defaultVaultId;
  ...
  const setDefaultVaultAction = useCallback(
    async (id: string) => {
      try {
        ...
        const fetchedVaults = await listVaults();
        const mappedVaults = sortVaultItems(fetchedVaults.map(mapVaultFromBackend));
        const nextDefaultVaultId =
          mappedVaults.find((vaultItem) => vaultItem.isDefault)?.id ?? DEFAULT_ACTIVE_VAULT_ID;

        setVaults(mappedVaults);

        if (!(settings?.multiply_vaults_enabled ?? false)) {
    ...
    async (id: string) => {
      try {
        ...
        setVaults((prev) => {
          ...
          nextDefaultVaultId =
            remaining.find((vaultItem) => vaultItem.isDefault)?.id ?? DEFAULT_ACTIVE_VAULT_ID;
          return remaining;
        });
